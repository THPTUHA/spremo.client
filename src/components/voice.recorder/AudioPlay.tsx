...
const AudioPlay = ({url,id}:{url: string,id: any})=>{
    ...
    return (
        <div ref={mp3Ref} className="w-full">
            {
                audio &&(
                     <>
                        <div className="flex justify-center items-center">
                            <span
                                ...
                                className={`cursor-pointer text-xl mr-5 ${is_looping ? "text-blue-500" : "text-gray-500"} hover:text-gray-900 transition-all`}>
                                <IoMdRepeat />
                            </span>
                            <span
                                onClick={() => goPrevXSeconds(true)}
                                className="cursor-pointer  text-xl shadow-md mr-5 text-white px-1 py-1 bg-blue-500 hover:bg-blue-500-dark transition-all rounded-full">
                                <FaChevronLeft />
                            </span>
                            <span onClick={() => togglePlay()} className="cursor-pointer   text-2xl shadow-md text-white px-2 py-2 bg-blue-500 hover:bg-blue-500-dark transition-all rounded-full">
                                {playing ? <IoMdPause /> : <FaPlay />}
                            </span>
                            <span
                                onClick={() => goNextXSeconds()}
                                className="cursor-pointer  text-xl shadow-md ml-5 text-white px-1 py-1 bg-blue-500 hover:bg-blue-500-dark transition-all rounded-full">
                                <FaChevronRight />
                            </span>
                            <select className=" cursor-pointer outline-none focus:outline-none text-lg ml-5 text-white hover:text-gray-900 transition-all" 
                        ...
                        <div className="px-4">
                            <div className="relative slider-container">
                                <span className="bar cursor-pointer shadow absolute left-0 top-3 w-full h-2 rounded-full overflow-hidden bg-blue-500 bg-opacity-30">
                                    <span className={`fill block w-0 h-full bg-blue-500 id${id}`}></span>
                                </span>
                                <input id="slider" type="range" min={0} max={Number.isNaN(audio.duration) ? 1 : Math.floor(audio.duration)} value={time_listen} onChange={onChangeHandle}
                                    className="slider cursor-pointer relative appearance-none w-full h-1 rounded-full outline-none bg-blue-500" />
                            </div>
                        </div>
                    </>
                )
